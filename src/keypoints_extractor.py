...
class EEFRetargeter:
    def compute_axes(self, joints_cam: np.ndarray, method: str,
                     axis_len: float, axis_scale: float) -> np.ndarray:
        ...
        for i in range(joints_cam.shape[0]):
            origin, R = self._compute_eef_frame(joints_cam[i], method, idx)
    ...
    def _compute_eef_frame(self, joints: np.ndarray, method: str, idx: Dict[str, int]) -> Tuple[np.ndarray, np.ndarray]:
        w = joints[idx["wrist"]]
...
class HandRetargetPipeline:
    ...
    def render_video(
        ...
        for m in method_list:
            axes_left = self.retargeter.compute_axes(left_cam, m, axis_len, axis_scale)
            axes_right = self.retargeter.compute_axes(right_cam, m, axis_len, axis_scale)
