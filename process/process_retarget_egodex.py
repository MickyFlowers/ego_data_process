...
def process_clip_remote(
    ...
    from src.retarget import EgoDexRetargetPipeline

    hdf5_path = Path(hdf5_path)
    hdf5_task_id = hdf5_path.parent.stem
    part_id = hdf5_path.parent.parent.stem
    output_dir = Path(output_dir)
    clip_id = f"{part_id}_{hdf5_task_id}_{hdf5_path.stem}"
    t0 = time.time()

    try:
        pipeline = EgoDexRetargetPipeline()
...
def main():
    parser = argparse.ArgumentParser(
        description="Ray parallel batch retarget (HDF5 -> JSON + optional replay video)"
    )
    parser.add_argument("--data-dir", type=str, default=None, help="Input dir (searched for .hdf5)。与 --clip-parts 二选一")
    parser.add_argument("--clip-parts", type=str, default=None, help="clip_parts.json 路径，从中读取 .hdf5 路径列表，替代文件夹扫描")
    ...
    def _clip_id_from_hdf5(p: Path) -> str:
        part_id = p.parent.parent.stem
        hdf5_task_id = p.parent.stem
        return f"{part_id}_{hdf5_task_id}_{p.stem}"

    if args.clip_parts and Path(args.clip_parts).is_file():
        paths = _load_paths_from_clip_parts(args.clip_parts)
        paths = [p for p in paths if str(p).lower().endswith(".hdf5")]
        seen: set[str] = set()
        files = []
        for p in paths:
            cid = _clip_id_from_hdf5(Path(p))
            if cid not in seen:
                seen.add(cid)
                files.append(Path(p))
        print(f"[Batch] --clip-parts: {len(files)} 个 .hdf5 路径")
    elif args.data_dir:
        input_dir = Path(args.data_dir).resolve()
        if not input_dir.is_dir():
            raise SystemExit(f"Input path is not a directory: {input_dir}")
        if args.no_recursive:
            candidates = sorted(input_dir.glob("*.hdf5"))
        else:
            candidates = sorted(input_dir.rglob("*.hdf5"), key=lambda p: (p.stem, str(p)))
        seen = set()
        files = []
        for p in candidates:
            cid = _clip_id_from_hdf5(p)
            if cid not in seen:
                seen.add(cid)
                files.append(p)
    else:
    ...
    for i, fp in enumerate(files):
        ...
        clip_ids.append(_clip_id_from_hdf5(fp))
